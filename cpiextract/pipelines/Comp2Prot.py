...
class Comp2Prot(Pipeline):
    ...
    def _update_args(self, pChEMBL_thres: float, stitch_stereo: bool,
                     otp_biblio: bool, dtc_mutated: bool, dc_extra: bool,
    ...
    def comp_interactions(
            self,
            input_id: int | str,
            pChEMBL_thres: float = 0,
            stitch_stereo: bool = True,
            otp_biblio: bool = False,
            dtc_mutated: bool = False,
            dc_extra: bool = False) -> tuple[pd.DataFrame, pd.DataFrame]:
        '''Collect data and merges all the data from the various sources together.'''
        # Run interaction select with all databases selected
        comp_tar, state = self.comp_interactions_select(
            input_id,
            pChEMBL_thres=pChEMBL_thres,
            stitch_stereo=stitch_stereo,
            otp_biblio=otp_biblio,
            dtc_mutated=dtc_mutated,
            dc_extra=dc_extra)
    ...
    def comp_interactions_select(
            self,
            input_id: int | str,
            selected_dbs: str = 'pc_chembl_bdb_stitch_ctd_dtc_otp_dc_db',
            pChEMBL_thres: float = 0,
            stitch_stereo: bool = True,
            otp_biblio: bool = False,
            dtc_mutated: bool = False,
            dc_extra: bool = False) -> tuple[pd.DataFrame, pd.DataFrame]:
        '''Collect data and merges all the data from the selected sources together.'''
        ...
        chembl_ids: list[str] = []
        self._update_args(pChEMBL_thres, stitch_stereo, otp_biblio,
                          dtc_mutated, dc_extra, chembl_ids)

        main_columns = [
            'entrez', 'gene_type', 'hgnc_symbol', 'description',
            'pchembl_value', 'datasource'
        ]
        ...
        if len(comp_ids) > 0:
            ...
            for name, db in self.databases.items():
                if name in db_list:
                    ...
                    result, state, _ = db.interactions(
                        comp_ids, *self.database_args.get(name, ()))
                    if len(result) > 0:
            ...
            if len(comp_tar) > 0:
                ...
                std_ids = comp_ids[[
                    'isomeric_smiles', 'inchi', 'inchikey', 'iupac_name'
                ]]
                # Select first nonNa value from each column
                std_ids = std_ids.apply(lambda col: std_ids[col.name].dropna(
                ).iloc[0] if not std_ids[col.name].dropna().empty else None)
    ...
    def _postprocess_databases(self, tar_all: pd.DataFrame) -> pd.DataFrame:
        ...
        tar_all = tar_all.dropna(subset=['hgnc_symbol', 'entrez']).reset_index(
            drop=True)
        ...
        comp_tar = pd.DataFrame(columns=[
            'entrez', 'hgnc_symbol', 'description', 'gene_type',
        ...
        for index, target in enumerate(tar_list):
            ...
            comp_tar.loc[index,
                         'hgnc_symbol'] = tar['hgnc_symbol'][tar.index[0]]
            comp_tar.loc[index,
                         'description'] = tar['description'][tar.index[0]]
            ...
            for source in self.sources:
                comp_tar.loc[index, source.lower(
                )] = 1 if source in tar['datasource'].unique() else 0
