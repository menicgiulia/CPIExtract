...
class Prot2Comp(Pipeline):
    ...
    def _update_args(self, pChEMBL_thres: float, stitch_stereo: bool,
                     dtc_mutated: bool, dc_extra: bool) -> None:
        self.database_args = {
            'pc': (pChEMBL_thres, ),
    ...
    def prot_interactions(
        ...
        prot_comp, state = self.prot_interactions_select(
            input_id,
    ...
    def prot_interactions_select(
        ...
        main_columns = [
            'inchi', 'inchikey', 'isomeric_smiles', 'iupac_name',
        ...
        if len(prot_ids) > 0:
            ...
            for name, db in self.databases.items():
                if name in db_list:
                    ...
                    db_comp, state, _ = db.compounds(
                        prot_ids, *self.database_args.get(name, ()))
                    if len(db_comp) > 0:
    ...
    def _postprocess_databases(self, comp_all: pd.DataFrame) -> pd.DataFrame:
        ...
        tar_comp = pd.DataFrame(columns=[
            'inchi', 'inchikey', 'isomeric_smiles', 'iupac_name',
            'pchembl_count', 'ave_pchembl', 'src_count'
        ] + [source.lower() for source in self.sources] + ['notes'])
        ...
        for index, compound in enumerate(comp_list):
            ...
            if len(comp) > 0:
                ...
                if len(indexes_with_N) > 0:
                    tar_comp.loc[index,
                                 'inchi'] = comp1['inchi'][indexes_with_N[0]]
                    tar_comp.loc[index, 'inchikey'] = comp1['inchikey'][
                        indexes_with_N[0]]
                    tar_comp.loc[index,
                                 'isomeric_smiles'] = comp1['isomeric_smiles'][
                                     indexes_with_N[0]]
                    tar_comp.loc[index, 'iupac_name'] = comp1['iupac_name'][
                        indexes_with_N[0]]
                    # tar_comp.loc[index,'pchembl_value'] = comp1['pchembl_value'][indexes_with_N[0]]
                else:
                    tar_comp.loc[index,
                                 'inchi'] = comp1['inchi'][comp.index[0]]
                    tar_comp.loc[index,
                                 'inchikey'] = comp1['inchikey'][comp.index[0]]
                    tar_comp.loc[index,
                                 'isomeric_smiles'] = comp1['isomeric_smiles'][
                                     comp.index[0]]
                    tar_comp.loc[index, 'iupac_name'] = comp1['iupac_name'][
            ...
            for source in self.sources:
                tar_comp.loc[index, source.lower(
                )] = 1 if source in comp['datasource'].unique() else 0
