        ...
        if len(cids) > 0:
            ...
            if len(cids) > 1000:
                for start, end in generate_subsets(len(cids), 1000):
                    ...
                    comps = call_with_retry(pc.get_compounds, subset, selected_columns, namespace='cid')
                    compounds = pd.concat([compounds, comps])
                    time.sleep(0.5)
            else:
                compounds = call_with_retry(pc.get_compounds, cids, selected_columns, namespace='cid')
        ...
        if len(inchikeys) > 0:
            ...
            for inchikey in inchikeys:
                try: # Get CIDs for this inchikey (or first-block)
                    cids = call_with_retry(pcp.get_cids, inchikey, namespace='inchikey', searchtype=None)
                    if cids:
            ...
            if len(all_cids) > 0: #batch-fetch the compound data for all CIDs
                if len(all_cids) > 1000:
                    ...
                    compounds = call_with_retry(pc.get_compounds, all_cids, selected_columns, namespace='cid')
                if len(compounds) > 0:
                    db_comps = compounds.rename(columns=pc.properties)
    ...
    def _pubchem_search_chembl(self, db_act: pd.DataFrame, id_name: str, columns: list[str], pc: PubChemServer) -> pd.DataFrame:
        ...
        for ids in db_act[id_name].unique():
            try:
                ...
                compound = call_with_retry(pc.get_compounds, ids, selected_columns, namespace='name')
        ...
        if len(missing_id) > 0:
            # clear to guard against mutated data from cut connection
            def _molecule_search_clean_retry():
                inchis.clear()
                chembl_ids.clear()
                return chembl.molecule_search(missing_id, inchis, chembl_ids)
            call_with_retry(_molecule_search_clean_retry)
        ...
        if len(inchis) > 0:
            ...
            for ids in zip(inchis, chembl_ids):
                try:
                    ...
                    pubchem_inchi.append(call_with_retry(pc.get_compounds, ids[0], selected_columns, namespace='inchi'))
